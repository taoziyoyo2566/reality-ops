...
- **2026-09-19 · 状态页增加分钟与小时视图。** 起因：操作者希望看到比按天更细的情况
  （[`plan-node-status-page`](../console/plan-node-status-page-2026-09-18.md) §10，2026-09-19 批准）。改动：`/s/<token>/status` 接受
  `?view=minute|hour|day`，默认显示最近 24 小时；内容仍全部来自 `status.json`，没有新的地址或写入。
  影响：没有新字段的 `status.json` 照旧只显示按天；页面仍不含脚本与节点连接信息，也不显示检测耗时。
