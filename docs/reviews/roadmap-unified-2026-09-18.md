# Reality Ops 统一建设与治理计划（2026-09-18 版）

Status: **DRAFT — 2026-09-18 操作者定下新方向：现有用户以后迁到新系统，旧系统不再修复既有问题（§2）。**

Updated: 2026-09-18 JST

取代：[`roadmap-unified-2026-09-16.md`](roadmap-unified-2026-09-16.md)（保留为历史版本；与 2026-08-27 版的差异见该文件 §0）。

## 0. 相对 2026-09-16 版的变化

| 项目 | 09-16 版 | 本版 |
|---|---|---|
| 旧用户去向 | 不迁移旧用户；新系统的订阅由管理员逐个发放 | **以后把现有用户全部迁到新系统**（分批，时间未定），迁移完成后停用旧系统（§2、§7 S7） |
| 旧系统的遗留问题 | C02–C04 的处理方式待决（§9 第 12 项） | 不再单独处置：旧系统维持现状，这些问题随迁移完成、旧系统停用而关闭（§9 第 12 项已决定） |
| C18（节点上的用户可经域名访问容器内监听端口） | 2026-09-18 新增，新旧系统都待处理 | 新数据面已修复并上线；旧系统同类路径按上一行处理，不检查、不修改（§4） |
| S7 | 新系统扩展：按需在其余节点部署新实例 | 新系统扩展与用户迁移：其余节点部署 + 分批迁移用户 + 旧系统停用（§7） |
| 完成定义 | 旧系统的去留不在范围内 | 全部用户在新系统上、旧系统停用后才算完成（§11） |
| U5 客户端防泄漏 / WebRTC | 待调查后确定做法 | **非优先**，推后处理，不进入近期实施顺序（§7.2） |
| 节点状态页 | 计划待批准 | 2026-09-18 已上线，4 台节点探测正常（§7 的 S6） |
...
**2026-09-18 调整（操作者决定）**：新系统的目标从“独立并行”改为**最终承接全部用户**：

- 现有用户以后迁到新系统，分批进行，时间未定（§7 的 S7）；
- 迁移完成后停用旧系统；在此之前旧系统维持现状，**不修复既有问题**（C02–C04、C06、C18 的旧系统部分），
  接受这段时间的残余风险（§9 第 12 项）；
- 迁移需要的用户管理能力由控制台 P2 提供（§7.0），到期与吊销的自动执行仍按 P3；
- 客户端防泄漏与 WebRTC（U5、C17）明确为**非优先**，推后处理（§7.2）；
...
| C02 | P0 | 33 份用户私钥位于已跟踪档案，仓库历史已暴露 | 2026-09-15 操作者决定不另做全量重签：新架构只保存节点本地私钥，订阅只输出公钥，新系统不含每用户私钥。2026-09-18 决定：不对旧系统做处置，**本项随用户迁到新系统、旧系统停用而关闭**（§9 第 12 项）；在此之前已暴露的旧私钥持续有效，属接受的残余风险 |
| C03 | P0 | 订阅链路共享 token、可枚举用户 ID、运行态密钥为空时代码层 fail-open | 新订阅服务直接返回正文；每用户独立不可枚举凭据；缺密钥拒绝；A 凭据不能取 B 内容；凭据可单独轮换与吊销。新系统已按此实现（S4）；**本项随用户迁到新系统、旧订阅停用而关闭**（2026-09-18，§9 第 12 项） |
| C04 | P0 | Gist ID 已公开，发布失败/跳过可能静默，旧用户文件不会自动删除，token 会进入输出 | 新系统不使用 Gist，也不以任何 ID 作为安全边界。旧 Gist 继续服务尚未迁移的用户；**本项随迁移完成后停用旧系统与旧 Gist 而关闭**（2026-09-18，§9 第 12 项），不单独整改 |
...
| C18 | P0 | 节点上用户可经“解析到 127.0.0.1 的域名”访问节点容器内的监听端口（metrics 10086，原理上还有 Xray API 10085）：`block-private` 只匹配字面 IP，`IPIfNonMatch` 在默认规则已匹配时不做解析 | `[实测]` 2026-09-18 本地 e2e：域名方式取到 `/debug/vars` 返回 200（各用户流量计数与用户名），字面 IP 被拦下。影响所有 `xray_edge` 节点；新系统当前只有 `test` 持有订阅。`[代码]` 2026-09-18 已部署到 4 台新系统节点：路由 `domainStrategy` 改为 `IPOnDemand`，使 `block-private` 在匹配前先解析域名；
`[实测]` 节点端到端 44/44，字面 IP、指向环回的域名、指向 Docker 网关的域名都被拦下，SOCKS5 分流与正常连接不变。旧系统（模板仍为 `IPIfNonMatch`）按 2026-09-18 决定不检查、不修改，随停用关闭 |
...
**进度总览**（更新于 2026-09-18；某阶段有进展时只改对应一行，证据见各阶段计划与 [`docs/project-memory.md`](../project-memory.md)）：
...
| 出口管理 | 第一期已部署（2026-09-19）；2026-09-20 补充分流方案、阻断 QUIC 与设计排查的修正，本地验证完成待部署 | [`plan-egress-console`](console/plan-egress-console-2026-09-19.md)：出口池存在控制台数据库（不写死在代码与部署配置），显示可用状态与出口 IP；按节点和用户分配，运行中生效不重启 Xray；出口失效时按开关回退直连或断开；类型按注册表扩展，第一期 SOCKS5。2026-09-20：分流方案、只有 TCP 且阻断 QUIC、UDP 检测、ECH 与 geo 数据等修正（[`review-egress-design`](console/review-egress-design-2026-09-20.md)）；jp10 的旧网站列表部署后作为第一个方案加入；操作步骤见 `operations.md` §14.13 |
...
| S7 新系统扩展与用户迁移（2026-09-18 恢复迁移目标） | 按需在其余节点部署新实例（与旧实例并行）；分批把现有用户迁到新系统：建立用户与凭据、发放新订阅地址、通知与观察、停用其旧订阅；全部迁完后停用旧系统 | 目标节点预检通过（443 空闲、Compose 插件、磁盘与内存）；每台节点单独授权；用户迁移另需控制台 P2 与单独的迁移计划（分批名单、回滚、通知）；2026-09-19 操作者决定旧系统不动、不做按用户的旧订阅停用，迁移以新系统可用为准（[`plan-user-migration`](console/plan-user-migration-2026-09-19.md)） | 目标节点完成新实例验收并出现在新订阅中；每个用户在新系统上可用（“使用中”）或已由操作者决定；旧系统何时停机另行决定；不可达节点明确列为 gap |
...
| 节点状态页（S6 的一部分） | 订阅用户查看所有新系统节点的当前状态与 90 天历史；管理员查看探测明细并给事件写说明 | P1；各节点增加只能连到检测地址的探测账号（S3 合同变更） | 已上线（2026-09-18），见 [`plan-node-status-page`](console/plan-node-status-page-2026-09-18.md) |
| P2 第二阶段 | 网页增删改用户、停用与到期、批量发放与导出；节点每分钟拉取用户名单自动生效；Telegram bot；认证预留（2026-09-18 调整） | 授权模型用“档位 + 例外”的最小集（沿用现有层级），在 P2 计划中定，不等完整的 S1 | 已批准（2026-09-18），见 [`plan-console-phase2`](console/plan-console-phase2-2026-09-18.md) |
...
**2026-09-18 操作者决定：非优先，推后处理**——不进入近期实施顺序，Shadowrocket 全局模式下 WebRTC 显示真实 IPv4 的调查
（原 §7.2 缺口）同样推后；下述合同在恢复优先级时仍然适用。
...
11. 是否删除旧 single/multi 实例、旧代码与旧每用户私钥：2026-09-15 决定迁移期间不删除；2026-09-16 操作者表示不急于删除；
    2026-09-18 决定用户迁完后停用旧系统，删除时机随迁移进度另定。
12. ~~旧系统的遗留 P0 风险如何处置~~：**2026-09-18 已决定**——不对旧系统做任何处置（包括 C02、C03、C04、C06 与 C18 的旧系统部分），
    把用户迁到新系统后停用旧系统，由此关闭；迁移完成前接受残余风险。
...
1. C00–C18 均关闭，或有操作者接受的明确残余风险与所有者（旧系统上的 C02–C04、C06、C18 按 §9 第 12 项随停用关闭）；
2. 控制面和数据面合同、测试、升级与恢复路径完成；
3. 全部用户在新系统上、经新订阅使用，管理控制台可以发放、吊销并查看节点与流量；旧系统已停用（§7 的 S7、§9 第 11 项）；
