...
| 出口管理 | 方案已批准（2026-09-19），第一期实现中 | [`plan-egress-console`](console/plan-egress-console-2026-09-19.md)：出口池存在控制台数据库（不写死在代码与部署配置），显示可用状态与出口 IP；按节点和用户分配，运行中生效不重启 Xray；出口失效时按开关回退直连或断开；类型按注册表扩展，第一期 SOCKS5 |
