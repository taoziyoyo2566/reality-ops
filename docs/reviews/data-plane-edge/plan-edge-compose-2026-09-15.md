...
Status: **APPROVED — 2026-09-15 操作者按 §8 建议决定 D-E1～D-E3；仅授权工作树内实现与 §6.1 本地验证（§9）。**
...
2026-09-15 操作者决定：全部按下列建议执行（canary 以 `docker save`/`load` 分发、应用器进入工具镜像、与 S4 本地实现并行）。
...
本文已于 2026-09-15 批准，仅授权工作树内实现与 §6.1 本地验证（含控制端构建镜像与本地 compose 运行），并在被修订合同 §10 追加变更记录。
