...
| 待处理问题 | C00–C17 | 新增 C18（节点上用户可经域名访问容器内监听端口，2026-09-18 实测） |
...
`[实测]` 节点端到端 44/44，字面 IP、指向环回的域名、指向 Docker 网关的域名都被拦下，SOCKS5 分流与正常连接不变。剩余：确认旧系统（`IPIfNonMatch`，模板 `roles/reality_single`、`reality_multi`）是否同样存在并决定是否处理 |
