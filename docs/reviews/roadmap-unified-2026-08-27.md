...
> **SUPERSEDED 2026-09-16.** 操作者决定新系统作为独立产品建设、不迁移旧用户，路线图另立为
> [`roadmap-unified-2026-09-16.md`](roadmap-unified-2026-09-16.md)，变化见其 §0。本文保留为此前的历史版本，不再更新；
> 引用本文的已批准计划，以新版为当前依据。
...
迁移期间禁止把架构切换与 VLESS Encryption、地址族或 REALITY target 变更混成同一波。
单实例、443 与节点级 REALITY 密钥互为前提，视为同一变更单元（§4.1.3 决定 3）。XHTTP 在新
实例内作为独立的第二步验证，通过后才迁移用户（决定 4，2026-09-15 修订）。

2026-09-15 起，数据面以**新增的独立实现**落地，现有 single/multi 代码与节点上的旧实例在全部
节点迁移完成前保持不动，迁移完成后整体删除（决定 7）。新实现先由新的 Ansible playbook/角色
管理，控制面与 agent 随后接管同一配置合同（决定 8）。
...
- `[实测]` 2026-09-15 00:00 JST 只读快照，inventory 13 台中 12 台可达（`ali` 以其 SSH 配置
  用户认证被拒）。仅输出汇总值：
...
| C02 | P0 | 33 份用户私钥位于已跟踪档案，仓库历史已暴露 | 2026-09-15 操作者决定不另做全量重签：新架构只保存节点本地私钥，订阅只输出公钥；旧每用户私钥随各节点 S7 contract 删除旧实例而失效，因此并行窗口应尽量短；迁移清单存放于私有载体 |
...
| C06 | P0 | 文件日志与 Docker stdout 缺可靠上界；`maxsize` 仅在 logrotate 被调用时检查；7 台节点未下发轮转配置（§3.3），`spt` 单文件 740MB，`jp05` 根分区 96%（主因不是日志） | 旧系统按决定 9 不修改：2026-09-15 数据显示 `spt` 日志所在分区剩余约 39GB，风险可接受，迁移前只观察。新实例的容器定义必须自带文件日志定时轮转（不依赖系统每日 logrotate）与 Docker 日志上界，作为 U1 合同项验收；`jp05` 磁盘另行排查 |
...
| C11 | P1 | 节点运行态和可达性不完整 | 接管前重新生成带日期的只读快照；未探测节点保持 gap。原 `[test_nodes]` 四台已于 2026-09-14 退役并移出 inventory，`hk-hn` 已改名 `hk01` 并有 SSH 映射；服务器已由操作者确认退掉，控制端订阅缓存已清理，Gist 已于 2026-09-14 23:56 JST 重新生成 |
...
- **XHTTP 在新实例内分两步（决定 4，2026-09-15 修订）**：第一步只上 443 RAW + REALITY +
  ...
  XHTTP 入站”），回落条件与真实客户端支持在该步 canary 中验证。两步都通过后才迁移用户。
...
| S4 订阅服务 | 每用户鉴权后直接渲染 raw VLESS；取消 Gist 安全依赖；按节点迁移状态输出旧链接或 443 链接（§7.1）；按 U5 下发防泄漏客户端配置（§7.2） | canonical node/public key 链路稳定；实施合同 [`plan-subscription-service`](subscription-service/plan-subscription-service-2026-09-15.md) 已批准（Docker Compose 形态） | A 不能取 B；吊销后内容不可取；无公开落点；日志和响应不泄密 |
...
| S7 现网迁移 | 按 §7.1 并行变更逐台接管节点，轮换订阅凭据；按对账结果分批迁移旧镜像节点 | C01–C12 的迁移阻塞项关闭；目标镜像 digest、回滚 digest 和告警规则已记录；回滚与用户通知批准 | 原 single 与原 multi 节点各至少一台完成 canary；所有节点部署新实例并在订阅中切到 443；33 用户完成迁移；所有可达节点为 `current`；不可达节点仍明确列为 gap；旧 Gist 与旧订阅凭据失效。**旧实例、旧代码与旧每用户私钥的删除不再属于本阶段退出门槛**（2026-09-15 决定，§9 第 11 项） |
...
| U4 XHTTP | 同一新实例内 443 fallback 到本地 XHTTP 入站 | S3 在 U1 验收后验证；S7 中在迁移用户前随新实例启用 |
...
5. **contract**：~~经操作者确认后删除旧容器~~ 2026-09-15 操作者决定：**迁移期间不删除旧实例与旧代码**，
   保留作为回退；全部用户迁到新订阅并观察稳定后，再单独决定是否删除（§9 第 11 项）。在删除之前：
   旧高位端口持续暴露，旧的每用户私钥持续有效（C02 未关闭），小内存节点长期并行两套实例，
   `edge_drift_old_only_users` 中登记的用户例外持续存在。
...
8. ~~S7 中 C03 订阅地址/凭据切换是否与节点迁移放在同一通知窗口~~：2026-09-15 决定**不同窗、先发地址**。S4 完成后即向用户发放
   新地址（对未迁移节点输出与旧 Gist 相同的链接），节点迁移随后随订阅刷新生效，不再通知（§7.1）；
9. ~~旧系统在 S7 前允许做哪些止血修复~~：2026-09-15 已决定不做修改（§4.1.3 决定 9）。
11. 迁移稳定后是否删除旧 single/multi 实例、旧代码与旧每用户私钥（2026-09-15 操作者决定暂不删除，保留回退）。
    关闭条件：全部用户在新订阅上稳定运行一段时间（时长由操作者定），并确认不再需要回退。
