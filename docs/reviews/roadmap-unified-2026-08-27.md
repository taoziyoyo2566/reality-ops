# Reality Ops 统一建设与治理计划

Status: **DRAFT — 整体重建方向已由操作者选定；实施合同与未决项尚待评审。**

Updated: 2026-09-14 JST

## 1. 文档职责

本文是本项目未来工作、当前问题、实施顺序与验收边界的**唯一当前事实源**。
...
迁移期间禁止把架构切换与 XHTTP、VLESS Encryption、地址族或 REALITY target 变更混成
同一波。先得到可验证的新控制回路，再迁移现网，最后扩展协议能力。单实例、443 与节点级
REALITY 密钥互为前提，视为同一变更单元（§4.1.3 决定 3）。
...
- `[缺口]` 除上述快照外，其余节点的 image、配置、权限、agent、磁盘、DNS、客户端兼容与订阅
  行为均未刷新。

## 4. 当前待处理问题

以下只保留仍影响当前安全、迁移或未来架构的事项。历史上已修复但未部署的行为仍按
“仓库已修 / 线上未知”区分。
...
| C01 | P0 | 控制端 `/tmp/reality_build` 有 world-readable 私钥配置残留（§3.3） | `[代码]` 2026-09-14 multi 构建已改为每主机 `tempfile` 私有目录（`0700/0600`），`always` 中删除；本地探针验证成功、失败注入与 `--tags config` 均无残留，尚未在真实 multi 部署中验证。剩余：经单独主机变更授权后清理旧残留，并在下一次 multi 部署后确认控制端无残留 |
| C02 | P0 | 33 份用户私钥位于已跟踪档案，仓库历史已暴露 | 设计并执行全量重签；新架构只保存节点本地私钥；旧密钥失效；迁移清单存放于私有载体 |
| C03 | P0 | 订阅链路共享 token、可枚举用户 ID、运行态密钥为空时代码层 fail-open | 新订阅服务直接返回正文；每用户独立不可枚举凭据；缺密钥拒绝；A 凭据不能取 B 内容；凭据可单独轮换与吊销 |
| C04 | P0 | Gist ID 已公开，发布失败/跳过可能静默，旧用户文件不会自动删除，token 会进入输出 | 旧 Gist 必须失效而非“评估是否轮换”；新 ID 不再作为安全边界；过渡修复必须非零失败并消除日志中的 token |
...
| C06 | P0 | 文件日志与 Docker stdout 缺可靠上界；`maxsize` 仅在 logrotate 被调用时检查 | 先止血磁盘风险；为文件日志建立明确调用频率，为 Docker 日志设上界；验证轮转、压缩、重启与回滚 |
| C07 | P0 | 现有 ACL 默认放行，档位与能力混用，显式 hosts 旁路；到期没有执行者 | 建立 `grade + features + 有时限例外` 的默认拒绝模型；逐用户映射在私有清单完成；旧范围与新授权逐项 diff |
...
| C11 | P1 | 节点运行态和可达性不完整 | 接管前重新生成带日期的只读快照；未探测节点保持 gap。原 `[test_nodes]` 四台已于 2026-09-14 退役并移出 inventory，`hk-hn` 已改名 `hk01` 并有 SSH 映射；控制端订阅缓存残留已清理，退役节点上的容器与已发布 Gist 内容尚未清理 |
| C12 | P1 | 旧镜像消费仍有漂移风险：浮动 `latest`、不会稳定重拉、新镜像仅在 3 台节点确认运行（§3.3）；项目期望镜像与节点实际运行镜像没有持续对账 | 固定已验证 digest；修正拉取策略；持续解析项目引用并采集节点实际 Image ID/RepoDigest/Xray 版本；canary 验证 geodata、坏配置、UID、健康检查和回滚；将节点标记为 `current`、`stale`、`wrong_repo`、`wrong_version`、`unreachable` 或 `unknown` |
| C13 | P1 | 订阅 URI/JSON/YAML 多处手工拼接，fragment 未按上游分享标准编码 | 建立单一 canonical node 与 renderer；使用标准序列化；golden 明确允许的编码修复；输出不含秘密 |
| C14 | P1 | 现有 Xray 配置仍缺 DNS/sniffing 等能力，multi stats 与监控模型不一致 | 迁移后按独立功能阶段处理；先定义合同与测试，禁止与控制面切换同批 |
...
| C16 | P1 | 测试基础不足以支撑安全迁移 | 建立授权纯函数、schema、renderer、agent 收敛、订阅鉴权、两个 core config check 和失败注入测试 |
...
- `[上游]` v26.3.27 release note：“非 443 端口、‘偷苹果’极易导致服务器 IP 被封锁”，并对 REALITY
  ...
  `.ru`/`.ir`/`.cn`。
- `[代码]` 现状每用户一个高位端口；默认 target 为 apple；`hk01` 用 icloud；`ali` 使用默认
  apple。
...
target 迁离 apple/icloud（C15，独立变更单元）；预留 XHTTP 入口布局（只预留，§4.1.3 决定 4）；用户档案去掉 `port`、`private_key`、
...
- **XHTTP 只预留（已决定）**：S7 不部署 XHTTP 入站，也不输出 XHTTP 订阅。结构上预留两点：
  期望状态与 agent 按“承载用户的入站列表”同步 clients，而不是写死单个入站；443 入站保留
  `fallbacks` 扩展位。`[推论]` 后续在 S8 新增本地监听的 `vless-xhttp` 入站，使用同一组 clients，
  由 443 入站回落到它（上游讨论 #4113：“REALITY 加回落……同一 path 抵达同一 XHTTP 入站”），
  具体回落条件在 S8 staging 验证；
...
| 3 | 按并行变更切换，不修订 §2 分波约束 | 依据见下方“切换方式”；变更单元、步骤与回滚归 §7.1 |
| 4 | XHTTP 只预留 | S7 只上线 443 RAW + REALITY + Vision；按上文预留入站列表与 `fallbacks` 扩展位；XHTTP 仍归 S8 |
...
- `[缺口]` 其余节点的 443 占用、Docker 版本与资源余量；bridge 下 IPv6 客户端源地址是否保留
  ...
  Vision 回落 XHTTP 随决定 4 移入 S8，不阻塞 C00。
...
- 迁移不能用一个标量 grade 假装复现现有非阶梯式访问范围；应先生成精确的临时节点 allowlist，
...
| S3 单节点数据面 | 一台全新或确认可用的测试节点完成本地密钥与原子收敛 | 测试节点确定；S2 通过 | 配置检查、起停、重启、断网、坏包、last-good、到期和回滚通过；按 §7.1 与旧实例并行运行，API 增删改用户、SOCKS5 成员变更与并行回滚通过 |
| S4 订阅服务 | 每用户鉴权后直接渲染 raw VLESS；取消 Gist 安全依赖；按节点迁移状态输出旧链接或 443 链接（§7.1） | canonical node/public key 链路稳定 | A 不能取 B；吊销后内容不可取；无公开落点；日志和响应不泄密 |
| S5 授权生命周期 | 到期、吊销、计划变更持续收敛 | 私有迁移映射已由操作者确认 | 在线 SLA 与离线 TTL 均达标；节点与订阅同时撤销；审计完整 |
...
| S7 现网迁移 | 按 §7.1 并行变更逐台接管节点，轮换全部旧密钥和订阅凭据；按对账结果分批迁移旧镜像节点 | C01–C12 的迁移阻塞项关闭；目标镜像 digest、回滚 digest 和告警规则已记录；回滚与用户通知批准 | 原 single 与原 multi 节点各至少一台完成 canary；所有节点完成 contract；33 用户完成迁移；旧仓库运行实例为 0；所有可达节点为 `current`；不可达节点仍明确列为 gap；旧凭据、旧 Gist 和旧控制链路失效 |
| S8 功能演进 | Mihomo、IPv4/IPv6、target、XHTTP、metrics、VLESS Encryption | S7 稳定；每项独立批准 | 按真实客户端/core 矩阵逐项验收；不得用解析成功代替真实连接 |
...
| 单元 | 内容 | 阶段 |
|---|---|---|
| U1 数据面 | 单实例、443、节点级 REALITY 密钥、`conf.d` 配置、API 用户管理（互为前提，不可拆分） | S3 验证，S7 逐台执行 |
| U2 target | 迁离 apple/icloud 等高风险 target（C15） | 节点完成 U1 contract 后单独 canary |
| U3 订阅凭据 | 每用户订阅凭据与新订阅地址（C03） | S4 实现；与节点迁移是否同窗通知见 §9 第 8 项 |
| U4 XHTTP | 443 fallback 到本地 XHTTP 入站 | S8 |

**单节点步骤（U1）**

1. **预检**：443 是否被占用、Docker 版本、磁盘与内存余量、bridge 下 IPv4/IPv6 源地址是否保留；
   任一项失败则该节点停止，记为 gap。
2. **expand**：与旧容器并行启动新实例（443）；旧的每用户高位端口继续服务；两边使用同一 UUID
   与 email（`<用户名>.<节点名>`），monitor 同时采集新旧实例。
3. **migrate**：订阅对该节点只输出 443 链接；客户端刷新后迁移；用 `/subs/logs` 找出长期未拉取
   订阅、可能手工导入链接的用户，单独通知。
4. **观察**：新实例各用户有流量，旧实例各用户流量持续为零；观察时长由操作者按节点批准。
5. **contract**：经操作者确认后删除旧容器、端口与旧配置；旧的每用户私钥随之失效（C02）。
   并行窗口应尽量短，因为旧高位端口本身就是封锁风险。
...
| migrate / 观察 | 订阅恢复输出旧链接；旧实例仍在运行 |
| contract 之后 | 不可回退到旧链路；只能修复新实例或从 last-good 恢复 |

**canary 顺序**

staging 节点 → 一台原 single 生产节点 → 一台原 multi 生产节点 → 其余节点分批。每台节点独立
执行与回滚；前一台未完成 contract 或出现未解释的异常时，不开始下一批。
...
- 迁移：旧/新授权与订阅 diff、节点应用版本、用户通知、旧凭据失效；
...
4. 新的测试节点从哪里来（S3 进入条件与 §7.1 canary 首站依赖它）；原 `[test_nodes]` 四台已于
   2026-09-14 决定退役；
5. 控制面部署形态、备份/恢复目标和高可用需求；
6. enrollment、签名算法、根密钥保管和轮换方式；
7. agent 升级由 Ansible、签名包还是其他机制负责；
8. S7 中 C03 订阅地址/凭据切换（唯一需要用户手动操作的一步）是否与节点迁移放在同一通知窗口；
   节点密钥与端口变化随订阅刷新生效，不需要通知（§7.1）；
9. 旧系统在 S7 前允许做哪些止血修复，哪些功能改动明确冻结。

## 10. 风险与回滚边界

- 新旧系统在 S7 完成前并存；不得提前删除旧入口或旧配置；
- 单个 canary 只改变一个轴；架构、image、address、target、transport、encryption 分开；单实例、
  443 与节点级密钥是同一变更单元 U1（§7.1）；
- 旧实例在 contract 前保留；contract 后旧每用户私钥失效，不能回退到旧链路（§7.1）；
...
1. C00–C16 均关闭，或有操作者接受的明确残余风险与所有者；
2. 控制面和数据面合同、测试、升级与恢复路径完成；
3. 所有现网节点和用户完成迁移，旧私钥、共享 token、Gist 落点和旧控制链路失效；
4. 授权、到期、吊销在在线与离线场景中按合同收敛；
5. 原 single/multi 节点与真实客户端矩阵通过，未执行项仍明确为 gap；
