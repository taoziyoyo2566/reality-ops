...
| S4 订阅服务 | 每用户鉴权后直接渲染 raw VLESS；取消 Gist 安全依赖；按节点迁移状态输出旧链接或 443 链接（§7.1）；按 U5 下发防泄漏客户端配置（§7.2） | canonical node/public key 链路稳定 | A 不能取 B；吊销后内容不可取；无公开落点；日志和响应不泄密 |
...
- `[缺口]` TUN 接管、真实设备上的 ipleak 类检测、v2rayN 与 Shadowrocket 的等价隐私配置均未验证。
