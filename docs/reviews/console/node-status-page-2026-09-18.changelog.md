...
- ~~计划 §6.3 的退出条件（连续 24 小时无误报）~~：2026-09-19 通过。近 24 小时 1440 轮、相邻两轮间隔都不超过 3 分钟
  （期间控制台多次重新部署），4 天内没有故障或部分异常事件，管理首页没有误报。
- ~~`legend` 偶发慢探测疑似 DNS 超时~~：2026-09-19 排查后排除。近 3 天超过 2 秒的探测集中在 dzire（56 次）、
  legend（69 次）、netcup（37 次），耗时多在 2–4 秒，没有集中在 5 秒或 10 秒（DNS 超时的特征）。节点上解析域名首次
  50–80ms、之后 2–20ms。spt 到这三台的 TCP 往返约 265、265、170ms（usca、dcc 约 30ms）：探测需要约 4 次往返，
  基础耗时约 1 秒；长距离线路偶发丢包重传，就会多出 1–3 秒。这是探测位置（spt）到节点的线路特性，不需要改节点配置。
