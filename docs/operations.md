...
| 要做的事 | 在哪里 | 说明 |
|---|---|---|
| 新增用户 | 用户 → 新增用户 | 填用户名、档位、单独允许 / 禁止的节点、到期日、备注；UUID 自动生成。节点上的账号在下次 `edge.yml` 后生效（§14.10） |
| 修改、停用、启用、删除用户 | 用户详情 | 停用与到期立即暂停订阅地址；节点上的变化在下次 `edge.yml` 后生效，页面标“待部署” |
| 给用户发订阅地址 | 用户 → 点用户名 → 发放订阅地址 | 页面显示地址与二维码，只发给本人；停用或到期的用户不能发放 |
| 批量发放、导出清单 | 用户列表勾选 → 发放并导出 | 已发放的沿用原地址；清单只在页面上显示和下载，不保存在服务器上 |
| 改节点的档位 | 节点详情 → 档位与用户 | 页面显示保存后下次部署会增加 / 移除哪些用户 |
...
- 把控制台里的用户变化落到节点上：运行 `edge.yml`（第二阶段 2b 上线后由节点自动完成）。
...
**日常**：在网页上新增或修改用户后运行 `edge.yml`，节点账号随之变化（控制台首页与用户页显示“待部署”）；
停用、到期与删除对订阅地址立即生效。新用户统一使用控制台生成的共用 `short_id`（D-P2-5），首次出现在某节点时该节点 Xray 重启一次。
