...
| 一批用户迁到新系统 | 用户列表勾选（表头的复选框全选当前列表）→ 发放并生成 Telegram 绑定链接 | 未发放的先发放；链接清单页附消息示例；进度看首页“迁移进度”（§14.12） |
...
| 让某些用户、网站或整台节点经指定代理出口 | 出口 → 添加出口（常用的网站组合存为“分流方案”）；节点详情 → 出口 → 分配 | 出口失效时按分配回退直连或断开；开启同步的节点约一分钟内生效，不重启 Xray（§14.13） |
| 节点维护时暂时不给用户，或把新节点加入订阅 | 节点列表 → 该行“隐藏 / 显示”，或勾选多台（表头的复选框全选）后“隐藏所选 / 显示所选” | 立即重新发布，用户刷新订阅后生效；节点详情页也有同样的按钮 |
...
| 首页“节点 X 的出口规则应用失败：…” | 节点 agent 用 Xray API 加规则时出错。看节点页“出口”一节的原因；节点 agent 过旧时运行 `edge.yml`（§14.13） |
...
合同：[`plan-egress-console`](reviews/console/plan-egress-console-2026-09-19.md)（2026-09-20 补充见其 §10）；
设计排查：[`review-egress-design-2026-09-20`](reviews/console/review-egress-design-2026-09-20.md)。
出口、账号密码、分流方案和分配都存放在控制台数据库，代码与部署配置里不写具体出口；第一期支持 SOCKS5。
需要节点开启用户同步（§14.10）；未开启同步的节点只在运行 `edge.yml` 时写入出口，不做检测与切换。

1. **出口 → 添加出口**，或在“批量导入”里每行一个 `socks5://账号:密码@地址:端口#名称`。spt 约一分钟内检测一次
   （检测前显示“等待检测”），出口页显示是否可用、UDP 是否支持、出口 IP、耗时。要马上知道结果，点该行或出口详情页的
   **立即检测**：可用的几秒内返回，失效的约 10 秒（只试一次，偶然失败可以再点）。
2. **出口 → 分流方案**（按网站分流时推荐）：填名称、网站、IP 段、网络、出口失效时。
   - 网站：直接写域名（`chatgpt.com`，表示该网站及其子域名），或 `domain:`、`full:`、`keyword:`、`geosite:` 写法；
     IP 段：CIDR 或 `geoip:`。网站和 IP 段满足任一项即走出口。保存前由 Xray 检查，拼错的 `geosite:` / `geoip:` 代码会被拒绝。
   - 修改方案后，用到它的各节点约一分钟内跟着变；有分配在用的方案不能删除。
3. **节点详情 → 出口 → 分配**：选出口，再选方案，或直接填网站、IP 段；需要时再勾选用户（不勾选 = 这台节点的所有用户）。
   都不填表示整台节点的全部流量。用户一栏只列出这台节点上的用户（按档位规则与用户的单独允许 / 禁止）；
   要给其他用户用，先在用户页允许这台节点。优先级数字小的先匹配，同一个连接只走第一条匹配的分配。
   - **网络**：
     - 只有 TCP（UDP 直连）：UDP 仍从节点直连；
     - **只有 TCP，阻断 QUIC**：按网站或 IP 段分流时推荐。这些目标的 QUIC（UDP 443）被阻断，浏览器改用 TCP，
       网站只看到出口的 IP；其他 UDP 照常直连。不能用于整个用户或整台节点；
     - TCP 和 UDP：要求出口支持 UDP；最近一次检测不支持 UDP 的出口不能这样分配。
   - **出口失效时**：回退直连（与没有这条分配相同，适合一般网站）或断开（要求网站只看到出口 IP 时，宁可断开也不换 IP）。
4. 开启同步的节点约一分钟内生效，不重启 Xray。节点页“出口”一节显示每条分配的现状：经出口 / 出口失效，已回退直连 /
   出口失效，已断开 / 同步中 / 未生效（附原因，例如所选用户都不在这台节点上、节点 agent 需要更新）。
   节点应用规则失败时，节点页与首页显示原因。用户详情“节点”表的“出口”列显示该用户在各节点走哪个出口。

检测与切换：

- 节点 agent 每次同步（60 秒）经分配到本节点的每个出口访问检测地址（`console_egress_check_url`，默认 Cloudflare 的 trace 页面），
  两次尝试都失败即判定失效，按分配回退直连或断开；连续两次成功后切回。TCP 可用时再经出口发一次 DNS 查询检测 UDP；
  选了“TCP 和 UDP”的分配在 UDP 不通时同样按失效处理。从出口失效到切换约为检测间隔 60 秒加检测本身的耗时
  （失效的出口两次各 10 秒超时，出口多时依次检测）。节点的检测结果显示在出口页，失效时首页提醒。
- 没有分配到节点的出口由 spt 检测（状态服务里的独立线程，不影响节点状态检测）：新加或修改后约一分钟内，之后每 10 分钟；
  “立即检测”也从 spt 检测。spt 的结果只说明 spt 能否连到代理；代理限制来源 IP 时 spt 上会显示不可用，以节点的检测为准。
- Xray 重启后，运行中的改动暂时丢失：reporter 连续读不到 Xray 后重启（约 1.5–2.5 分钟），其间按上次 `edge.yml` 写入的配置运行。

网站仍可能看到节点 IP 的情况（按网站分流的限制）：

- 客户端用 TUN 模式、把域名解析成真实 IP，同时浏览器开着 ECH：节点拿不到网站名，按网站分流不生效。客户端改用 fake-ip
  或代理模式即可；节点已处理代理模式下的 ECH（入站嗅探不采用 `cloudflare-ech.com` 这个外层名）。
- WebRTC 可以直接连 IP，不经网站名匹配；在意的用户应在浏览器中限制 WebRTC。
- Xray 重启后的空窗期（见上）。

其他：

- 修改出口时密码框留空表示不改。页面、操作记录和日志都不显示密码；节点只收到分配给自己的出口。
- 停用出口：选“回退直连”的分配改为直连；选“断开”的分配阻断这部分流量。删除出口：同时删除它在各节点上的分配，
  这些流量全部直连（包括选了“断开”的）。删除或修改后的旧账号密码留在节点配置文件里，直到下次运行 `edge.yml`。
- 从旧的 `socks5.yml` profile 过渡：用户来源为控制台（`edge_user_source: console`）时，`edge.yml` 不再把 `socks5.yml` 的 profile
  写入新系统节点。需要的出口先在出口页添加并分配，再运行 `edge.yml`。
- 回滚：在节点页删除或停用分配，约一分钟内恢复直连。
